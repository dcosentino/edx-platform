...
#from numpy.core.setup import val
...
def validate_professional_fields(post_vars):
    ...
    if post_vars.get('license_country') == 'United States':
        if len(post_vars.get('license_state')) < 2:
            error['success'] = False
            error['value'] = 'Enter your license state'
...
def setup_sub_affiliation_field(post_vars):
    ...
    affiliation_values = [
                          {'Packard Children\'s Health Alliance': 'PCHA_affiliation'},
...
def validate_affiliation_fields(post_vars):
    ...
    required_fields_list = [
                            {'sunet_id': 'Enter you SUNet ID'},
                            {'stanford_department': 'Enter your Stanford department'},
                           ]
...
def validate_required_secondaries(post_vars):
    ...
    required_secondaries_dict = {
   #                              'stanford_affiliated': ('1', 'how_stanford_affiliated', 'Choose how you are affiliated with Stanford.'),
